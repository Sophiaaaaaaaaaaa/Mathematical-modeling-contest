...
import os, sys, sets, random, string  # 调入基本的python模块
from string import *
from math import *
import pylab as pl
...
class BACA:  # 定义类BACA，执行蚁群基本算法
    def __init__(self, cityCount=101, antCount=101, q=80, alpha=1, beta=3, rou=0.4, nMax=100):
        # self, cityCount=51, antCount=50, q=80, alpha=2, beta=5, rou=0.3, nMax=40
        # 初始化方法，antCount为蚂蚁数，nMax为迭代次数
        self.CityCount = 10  # 城市数量，本例中为手工输入，也可以根据城市数据列表编写程序获得
    ...
    def ReadCityInfo(self, fileName):  # 定义读取城市文件的方法
        file = open('eil51.tsp')  # 打开城市文件
        for line in file.readlines():  # 逐行读取文件
            #cityN, cityX, cityY = string.split(line)  # 分别提取城市序号、X坐标和Y坐标，使用空格切分
        ...
        print(self.CityCount)
        for row in range(self.CityCount):  # 循环总城市数的次数
            distanceList = []  # 建立临时储存距离的空列表
            for col in range(self.CityCount):

                distance = sqrt(pow(CityList[row][1] - CityList[col][1], 2) + pow(CityList[row][2] - CityList[col][2],2))  # 逐一计算每个城市到所有城市的距离值 distance = round(distance)


                distance = round(distance)
                distanceList.append(distance)  # 追加一个城市到所有城市的距离值
            CityDistanceList.append(distanceList)  # 追加每个城市到所有城市的距离值，为矩阵，即包含子列表
        ...
        for iter in range(self.Nmax):  # 循环指定的迭代次数
            ...
            for ant in AntList:  # 循环遍历蚂蚁列表，由self.PutAnts()方法定义获取
                for ttt in range(len(CityList)):  # 循环遍历城市总数次数
                    ant.MoveToNextCity(self.Alpha,
                                       self.Beta)  # 执行蚂蚁的ant.MoveToNextCity()方法，获取蚂蚁每次旅行时的旅行路径长度CurrLen，禁忌城市城市列表TabuCityList等属性值
...
class ANT:  # 定义蚂蚁类，使得蚂蚁具有相应的方法和属性
    ...
    def UpdatePathLen(self):  # 定义更新旅行路径长度方法
        ...
        self.CurrLen = self.CurrLen + CityDistanceList[lastCity - 1][
            firstCity - 1]  # 将最后一个城市与第一个城市的距离值加到当前旅行路径长度，获取循环全部城市的路径长度
    ...
    def two_opt_search(self):
        ...
        for i in range(cityNum):
            for j in range(cityNum - 1, i, -1):
                # for j in xrange(i+1, cityNum):
                # for j in xrange((i+10) if (i+10)<cityNum else cityNum-1, i, -1):
